...
public class EventResource {
    ...
    public ResponseEntity<Event> createEvent(@Valid @RequestBody Event event, Principal principal) throws URISyntaxException {
        ...
        if (!Objects.equals(event.getOrganiser().getLogin(), principal.getName())){
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(ENTITY_NAME, "wronglogin", "Logged user must be organiser")).body(null);
        }
    ...
    public ResponseEntity<Void> deleteEvent(@PathVariable Long id, Principal principal) {
        ...
        Event event = eventService.findOne(id);
        if (!Objects.equals(event.getOrganiser().getLogin(), principal.getName())){
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(ENTITY_NAME, "wronglogindelete", "Only organiser can delete event.")).body(null);
        }
